CREATE TABLE IF NOT EXISTS omd_profile.dataset_profile
(
...
COMMENT ON TABLE omd_profile.dataset_profile IS
...
COMMENT ON COLUMN omd_profile.dataset_profile.dataset_profile_id IS
'Unique identifier of the dataset profiling record.';

COMMENT ON COLUMN omd_profile.dataset_profile.dataset_id IS
'Reference to the dataset in the Operational Metadata Model (OMD Core) that has been profiled.';

COMMENT ON COLUMN omd_profile.dataset_profile.row_count IS
'Total number of records analysed during the profiling process.';

COMMENT ON COLUMN omd_profile.dataset_profile.column_count IS
'Total number of columns identified in the dataset.';

COMMENT ON COLUMN omd_profile.dataset_profile.duplicate_rows IS
'Number of duplicated records detected across the analysed dataset.';

COMMENT ON COLUMN omd_profile.dataset_profile.profile_json IS
...
COMMENT ON COLUMN omd_profile.dataset_profile.profiled_at IS
'Timestamp indicating when the profiling process was executed.';



CREATE TABLE IF NOT EXISTS omd_profile.column_profile
(
...
COMMENT ON TABLE omd_profile.column_profile IS
...
COMMENT ON TABLE omd_profile.sample_data IS
