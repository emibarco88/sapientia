    ...
    <AppShell>
      <div className="vnext-page vnext-domain-page">
        <header className="vnext-domain-hero">
          ...
          <div className="vnext-domain-actions"><Link href={`/workspace/${domain}/explorer`} className="vnext-secondary-button"><Network size={17} /> Explore relationships</Link><Link href={`/workspace/${domain}/ai`} className="vnext-primary-button"><Sparkles size={17} /> Ask Sapientia</Link></div>
        </header>

        {error && <div className="vnext-alert">{error}</div>}
        {loading ? <div className="vnext-loading">Understanding {name}…</div> : (
          <>
            <AskBar domain={domain} prompt={`Ask anything about ${name}…`} />
